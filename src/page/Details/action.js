...
export const getTokenActions = (
  ...
  gpost(config.client.searchApi, params)
    .then(res => {
      if (res && res.find_fibos_tokens_action) {
        if (!!sucCb) {
          sucCb(res.find_fibos_tokens_action);
        }
      }
    })
    .catch(err => {});
  ...
  gpost(config.client.searchApi, count_params)
    .then(res => {
      if (res && res.count_fibos_tokens_action) {
        if (!!countSucCb) {
          countSucCb(res.count_fibos_tokens_action);
        }
      }
    })
    .catch(err => {});
